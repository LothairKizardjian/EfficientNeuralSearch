...
class Model(object):
    def __init__(
        ...
        with tf.device("/cpu:0"):
            ...
            def _pre_process(x):
                ...
                modif = 8 # Initially 32
                # --------------------------------------------------------------------------------
                # --------------------------------------------------------------------------------

                x = tf.pad(x, [[4, 4], [4, 4], [0, 0]])
                x = tf.random_crop(x, [modif, modif, 7], seed=self.seed)
                x = tf.image.random_flip_left_right(x, seed=self.seed)
                if self.cutout_size is not None:
                    mask = tf.ones([self.cutout_size, self.cutout_size], dtype=tf.int32)
                    start = tf.random_uniform([2], minval=0, maxval=modif, dtype=tf.int32)
                    mask = tf.pad(
                        mask,
                        [
                            [self.cutout_size + start[0], modif - start[0]],
                            [self.cutout_size + start[1], modif - start[1]],
                        ],
                    )
                    mask = mask[
                        self.cutout_size : self.cutout_size + modif,
                        self.cutout_size : self.cutout_size + modif,
                    ]
                    mask = tf.reshape(mask, [modif, modif, 1])
    ...
    def eval_once(self, sess, eval_set, feed_dict=None, verbose=False):
        ...
        if verbose:
            print("")
        print("{}_eaccuracy: {:<6.4f}".format(eval_set, float(total_acc) / total_exp))
