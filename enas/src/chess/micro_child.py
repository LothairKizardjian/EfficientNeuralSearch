...
from src.chess.image_ops import (
    batch_norm,
...
from src.chess.models import Model
...
class MicroChild(Model):
    ...
    def _factorized_reduction(self, x, out_filters, stride, is_training):
        ...
        with tf.variable_scope("path1_conv"):
            ...
            path1 = tf.nn.conv2d(
                path1, w, [1, 1, 1, 1], "VALID", data_format=self.data_format
            )
        ...
        with tf.variable_scope("path2_conv"):
            ...
            w = create_weight("w", [1, 1, inp_c, out_filters // 2])
            path2 = tf.nn.conv2d(
                path2, w, [1, 1, 1, 1], "VALID", data_format=self.data_format
            )
    ...
    def _apply_drop_path(self, x, layer_id):
        ...
        step_ratio = tf.cast(self.global_step + 1, tf.float32) / tf.cast(
            self.num_train_steps, tf.float32
        )
        ...
        with tf.variable_scope("calibrate"):
            ...
            if c[1] != out_filters:
                with tf.variable_scope("pool_y"):
                    ...
                    y = tf.nn.relu(y)
                    y = tf.nn.conv2d(
                        y, w, [1, 1, 1, 1], "SAME", data_format=self.data_format
                    )
    ...
    def _model(self, images, is_training, reuse=False):
        ...
        with tf.variable_scope(self.name, reuse=reuse):
            ...
            for layer_id in range(self.num_layers + 2):
                ...
                if (
                    self.use_aux_heads
                    and layer_id in self.aux_head_indices
                    and is_training
                ):
                    print("Using aux_head at layer {0}".format(layer_id))
                    with tf.variable_scope("aux_head"):
                        aux_logits = tf.nn.relu(x)
                        aux_logits = keras.layers.AveragePooling2D([5, 5], [3, 3], "VALID", data_format=self.actual_data_format)(aux_logits)
                    ...
                    aux_head_variables = [
                        ...
                        if (var.name.startswith(self.name) and "aux_head" in var.name)
                    ]
            ...
            if is_training and self.keep_prob is not None and self.keep_prob < 1.0:
                x = tf.nn.dropout(x, self.keep_prob) # Deprecated
                # x = tf.nn.dropout(x, rate=1-self.keep_prob)
            with tf.variable_scope("fc"):
                inp_c = self._get_C(x)
    ...
    def _fixed_combine(
        self, layers, used, out_filters, is_training, normal_or_reduction_cell="normal"
    ):
        """Adjust if necessary.
        ...
        with tf.variable_scope("final_combine"):
            for i, layer in enumerate(layers):
                if used[i] == 0:
                    ...
                    if hw > out_hw:
                        assert hw == out_hw * 2, "i_hw={0} != {1}=o_hw".format(
                            hw, out_hw
                        )
            ...
            with tf.variable_scope("cell_{}".format(cell_id)):
                ...
                with tf.variable_scope("x_conv"):
                    if x_op in [0, 1]:
                        ...
                        inp_c = self._get_C(x)
                        if x_op == 2:
                            x = tf.keras.AveragePooling2D([3, 3], [x_stride, x_stride], "SAME", data_format=self.actual_data_format)(x)
                        else:
                            x = tf.keras.layers.MaxPooling2D([3, 3], [x_stride, x_stride], "SAME", data_format=self.actual_data_format)(x)
                ...
                with tf.variable_scope("y_conv"):
                    if y_op in [0, 1]:
                        ...
                        inp_c = self._get_C(y)
                        if y_op == 2:
                            y = tf.keras.layers.AveragePooling2D([3, 3], [y_stride, y_stride], "SAME", data_format=self.actual_data_format)(y)
                        else:
                            y = tf.keras.layers.MaxPooling2D([3, 3], [y_stride, y_stride], "SAME", data_format=self.actual_data_format)(y)
                        if inp_c != out_filters:
                            w = create_weight("w", [1, 1, inp_c, out_filters])
                            y = tf.nn.relu(y)
                            y = tf.nn.conv2d(
                ...
                out = x + y
                layers.append(out)
        out = self._fixed_combine(
            layers, used, out_filters, is_training, normal_or_reduction_cell
        )
    ...
    def _enas_cell(self, x, curr_cell, prev_cell, op_id, out_filters):
        ...
        with tf.variable_scope("avg_pool"):
            avg_pool = tf.keras.layers.AveragePooling2D([3, 3], [1, 1], "SAME", data_format=self.actual_data_format)(x)
            ...
            if avg_pool_c != out_filters:
                with tf.variable_scope("conv"):
                    ...
                    avg_pool = batch_norm(
                        avg_pool, is_training=True, data_format=self.data_format
                    )

        with tf.variable_scope("max_pool"):
            max_pool = tf.keras.layers.MaxPooling2D([3, 3], [1, 1], "SAME", data_format=self.actual_data_format)(x)
            ...
            if max_pool_c != out_filters:
                with tf.variable_scope("conv"):
                    ...
                    max_pool = batch_norm(
                        max_pool, is_training=True, data_format=self.data_format
                    )
    ...
    def _enas_conv(
        self, x, curr_cell, prev_cell, filter_size, out_filters, stack_conv=2
    ):
                ...
                with tf.variable_scope("stack_{0}".format(conv_id)):
                    ...
                    with tf.variable_scope("bn"):
                        ...
                        offset = create_weight(
                            "offset",
                            [num_possible_inputs, out_filters],
    ...
    def build_valid_rl(self, shuffle=False):
        ...
        with tf.device("/cpu:0"):
            ...
            def _pre_process(x):
                ...
                # CIFAR10 to baduk modification
                x = tf.random_crop(x, [8, 8, 7], seed=self.seed)
