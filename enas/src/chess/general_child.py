...
class GeneralChild(Model):
    ...
    def _factorized_reduction(self, x, out_filters, stride, is_training):
        ...
        with tf.variable_scope("path1_conv"):
            ...
            path1 = tf.nn.conv2d(
                path1, w, [1, 1, 1, 1], "SAME", data_format=self.data_format
            )
        ...
        with tf.variable_scope("path2_conv"):
            ...
            w = create_weight("w", [1, 1, inp_c, out_filters // 2])
            path2 = tf.nn.conv2d(
                path2, w, [1, 1, 1, 1], "SAME", data_format=self.data_format
            )
    ...
    def _model(self, images, is_training, reuse=False):
        with tf.variable_scope(self.name, reuse=reuse):
            ...
            with tf.variable_scope("stem_conv"):
                w = create_weight("w", [3, 3, 7, out_filters])
                x = tf.nn.conv2d(
    ...
    def _enas_layer(self, layer_id, prev_layers, start_idx, out_filters, is_training):
        ...
        if self.whole_channels:
            ...
            with tf.variable_scope("branch_0"):
                y = self._conv_branch(
                    inputs, 3, is_training, out_filters, out_filters, start_idx=0
                )
            ...
            with tf.variable_scope("branch_2"):
                y = self._conv_branch(
                    inputs, 1, is_training, out_filters, out_filters, start_idx=0
                )
            ...
            if self.num_branches >= 5:
                with tf.variable_scope("branch_4"):
                    y = self._pool_branch(
                        inputs, is_training, out_filters, "avg", start_idx=0
                    )
                branches[tf.equal(count, 4)] = lambda: y
            if self.num_branches >= 6:
                with tf.variable_scope("branch_5"):
                    y = self._pool_branch(
                        inputs, is_training, out_filters, "max", start_idx=0
                    )
            ...
            # out = tf.case(
            #    branches, default=lambda: tf.constant(0, tf.float32), exclusive=True
            # )
            ...
            if self.num_branches >= 5:
                with tf.variable_scope("branch_4"):
                    branches.append(
                        self._pool_branch(
                            inputs, is_training, count[9], "avg", start_idx=count[8]
                        )
                    )
            if self.num_branches >= 6:
                with tf.variable_scope("branch_5"):
                    branches.append(
                        self._pool_branch(
                            inputs, is_training, count[11], "max", start_idx=count[10]
                        )
                    )
    ...
    def _fixed_layer(self, layer_id, prev_layers, start_idx, out_filters, is_training):
        ...
        if self.whole_channels:
            ...
            with tf.variable_scope("branch_2"):
                ...
                branches.append(self._conv_branch(inputs, 1, is_training, count[5]))
            with tf.variable_scope("branch_3"):
                total_out_channels += count[7]
                branches.append(
                    self._conv_branch(inputs, 1, is_training, count[7], separable=True)
                )
            if self.num_branches >= 5:
                with tf.variable_scope("branch_4"):
    ...
    def _conv_branch(
        ...
        with tf.variable_scope("inp_conv_1"):
            w = create_weight("w", [1, 1, inp_c, out_filters])
            x = tf.nn.conv2d(
                inputs, w, [1, 1, 1, 1], "SAME", data_format=self.data_format
            )
            x = batch_norm(x, is_training, data_format=self.data_format)
            x = tf.nn.relu(x)
